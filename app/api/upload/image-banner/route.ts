...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 現在のバナー画像数をチェック（最大3枚）
    const currentBannerCount = await prisma.userImageBanner.count({
      where: { userId }
    });

    if (currentBannerCount >= 3) {
      return NextResponse.json(
        { error: 'バナー画像は最大3枚までです' },
        { status: 400 }
      );
    }
    ...
    if (isSvgFile(file)) {
      ...
      // ファイル名を生成（SVGはそのまま）
      fileName = generateImageFileName(userId, 'user-banner', 'image/svg+xml');
      ...
      const processedResult = await processImageWithPreset(inputBuffer, 'banner');
      
      // ファイル名生成
      fileName = generateImageFileName(userId, 'user-banner', processedResult.outputFormat);
    ...
    // データベースに保存
    const bannerImage = await prisma.userImageBanner.create({
      data: {
        userId,
        imgUrl: fileUrl,
        sortOrder: sortOrder || currentBannerCount,
    ...
    return NextResponse.json({
      id: bannerImage.id,
      url: fileUrl,
      sortOrder: bannerImage.sortOrder,
      message: 'バナー画像のアップロードが完了しました',
      details: uploadDetails
    });

  } catch (error) {
    console.error('=== バナー画像アップロードAPIエラー ===', error);
    return NextResponse.json(
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // 削除対象の画像を取得
    const bannerImage = await prisma.userImageBanner.findUnique({
      where: { id: imageId }
    });

    if (!bannerImage || bannerImage.userId !== userId) {
      return NextResponse.json(
        { error: '画像が見つかりません' },
        { status: 404 }
      );
    ...
    if (bannerImage.imgUrl) {
      try {
        const fileName = bannerImage.imgUrl.split('/').pop();
        if (fileName) {
    ...
    // データベースから削除
    await prisma.userImageBanner.delete({
      where: { id: imageId }
    });

    // 削除後の並び順を調整
    const remainingImages = await prisma.userImageBanner.findMany({
      where: { userId },
    ...
    console.error('バナー画像削除エラー:', error);
    return NextResponse.json(
      { error: '削除に失敗しました' },
      { status: 500 }
    );
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (imageId && (url !== undefined || alt !== undefined)) {
      ...
      await prisma.userImageBanner.update({
        where: { id: imageId },
        data: updateData
      });
...
export async function GET(request: NextRequest) {
  try {
    ...
    const bannerImages = await prisma.userImageBanner.findMany({
      where: { userId },
      orderBy: { sortOrder: 'asc' }
    });
    ...
    console.error('バナー画像取得エラー:', error);
    return NextResponse.json(
      { error: '取得に失敗しました' },
      { status: 500 }
    );
