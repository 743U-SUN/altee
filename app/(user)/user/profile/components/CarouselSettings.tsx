...
export function CarouselSettings({ userId }: CarouselSettingsProps) {
  ...
  const fetchCarouselImages = async () => {
    try {
      const response = await fetch(`/api/upload/carousel?userId=${userId}`);
      ...
      data.images.forEach((img: CarouselImage) => {
        urls[img.id] = img.url || '';
        alts[img.id] = img.alt || '';
      });
      setTempUrls(urls);
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDrop: async (acceptedFiles) => {
      ...
      try {
        ...
        formData.append('sortOrder', carouselImages.length.toString());
                ...
                {isUploading ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-gray-500">
                      残り{3 - carouselImages.length}枚追加可能
                    </p>
                  </>
                )}
              </div>
