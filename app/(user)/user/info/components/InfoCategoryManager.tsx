...
export function InfoCategoryManager({ userId }: InfoCategoryManagerProps) {
  ...
  const [openAccordions, setOpenAccordions] = useState<{ [key: string]: string }>({});
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      // 新しいカテゴリのQ&A管理アコーディオンを自動で開く
      setOpenAccordions(prev => ({ ...prev, [result.category.id]: 'questions' }));
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    try {
      ...
      setOpenAccordions(prev => {
        ...
        delete newAccordions[categoryId];
  ...
  function SortableItem({ category, index, isOpen, onToggle }: SortableItemProps) {
    const {
      attributes,
      listeners,
      setNodeRef,
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              ...
              <Label className="text-sm font-medium">カテゴリ名（最大30文字）</Label>
            </div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              {tempNames[category.id]?.length || category.name.length}/30文字 · Q&A: {category.questions.length}個
            </p>
          </div>
          ...
          <div className="space-y-6">
            {categories.map((category, index) => (
              <SortableItem
                ...
                category={category}
                index={index}
                isOpen={openAccordions[category.id] === 'questions'}
                onToggle={(value) => 
                  setOpenAccordions(prev => ({ ...prev, [category.id]: value }))
                }
              />
            ))}
          </div>
