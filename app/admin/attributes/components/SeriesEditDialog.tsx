...
export function SeriesEditDialog({
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      manufacturerId: series.manufacturer.id,
      isActive: series.isActive,
    },
  });

  // seriesが変更されたらフォームをリセット
  useEffect(() => {
    form.reset({
      name: series.name,
      slug: series.slug,
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'シリーズの更新に失敗しました');
      }

      toast({
        title: '成功',
        description: 'シリーズを更新しました',
      });
      ...
      toast({
        title: 'エラー',
        description: error instanceof Error ? error.message : 'シリーズの更新に失敗しました',
        variant: 'destructive',
      });
    } finally {
